...
const listMatch = content.match(/export const listConsolidatedServices[\s\S]*?\];/);
if (listMatch) {
...
// Don't forget to also remove smartwatch from this file since I reverted it!
content = content.replace(/, Apple Watches e relógios multimarcas/g, '');
fs.writeFileSync('src/data/servicosConsolidadosData.ts', content);
